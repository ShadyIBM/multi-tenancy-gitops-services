...
INGRESS_EXTERNAL_HOST_AC=$(oc get ingress.config.openshift.io cluster -o jsonpath='{ .spec.domain }')
INGRESS_EXTERNAL_HOST_ASI=$(oc get ingress.config.openshift.io cluster -o jsonpath='{ .spec.domain }')
...
( echo "cat <<EOF" ; cat overrides-values.yaml_template ;) | \
...
INGRESS_EXTERNAL_HOST_ASI=${INGRESS_EXTERNAL_HOST_ASI} \
INGRESS_EXTERNAL_HOST_AC=${INGRESS_EXTERNAL_HOST_AC} \
sh > values.yaml
